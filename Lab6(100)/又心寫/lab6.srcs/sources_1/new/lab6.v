...
localparam [2:0] S_MAIN_INIT = 0, S_MAIN_PROMPT1 = 1,
                 S_MAIN_READ_NUM1 = 2, S_MAIN_REPLY = 3,
                 S_MAIN_PROMPT2=4,S_MAIN_READ_NUM2=5,
                 S_MAIN_DIVIDE=6;
...
reg  [0:PROMPT1_LEN*8-1] fir_msg = { "\015\012Enter the first decimal number: ", 8'h00 };
reg  [0:PROMPT2_LEN*8-1] sec_msg = { "\015\012Enter the second decimal number: ", 8'h00 };
reg  [0:REPLY_LEN*8-1]   quo_msg = { "\015\012The integer quotient is: 0x0000 \015\012", 8'h00 };
reg  [0:REPLY_LEN*8-1]   error_msg = { "\015\012ERROR! DIVIDE BY ZERO!          \015\012",8'h00};
reg  [15:0] quotient;  // The key-in number register
...
always @(posedge clk) begin
  if (~reset_n) begin
    for (idx = 0; idx < PROMPT1_LEN; idx = idx + 1) data[idx] = fir_msg[idx*8 +: 8];
    for (idx = 0; idx < PROMPT2_LEN; idx = idx + 1) data[idx+PROMPT1_LEN] = sec_msg[idx*8 +: 8];
  end
  else if (P == S_MAIN_REPLY) begin
    if(num2==0) for (idx = 0; idx < REPLY_LEN; idx = idx + 1) data[idx+PROMPT1_LEN+PROMPT2_LEN] = error_msg[idx*8 +: 8];
    else begin
    for (idx = 0; idx < REPLY_LEN; idx = idx + 1) data[idx+PROMPT1_LEN+PROMPT2_LEN] = quo_msg[idx*8 +: 8];
...
always @(*) begin // FSM next-state logic
  case (P)
    ...
    S_MAIN_READ_NUM2: // wait for <Enter> key.
      if (enter_pressed) P_next = S_MAIN_DIVIDE;
...
assign print_enable = (P != S_MAIN_PROMPT1 && P_next == S_MAIN_PROMPT1) ||
                  ...
                  (P == S_MAIN_DIVIDE && P_next == S_MAIN_REPLY);
...
assign tx_byte  = ((P == S_MAIN_READ_NUM1||P==S_MAIN_READ_NUM2||P==S_MAIN_INIT) && received)? echo_key : data[send_counter];
...
always @(posedge clk)begin
  if (~reset_n) num1 <= 0;
  else if (P == S_MAIN_INIT || P == S_MAIN_PROMPT1) num1 <= 0;
  else if (received && is_num_key&&P==S_MAIN_READ_NUM1) num1 <= (num1 * 10) + (rx_byte - 48);
end
always @(posedge clk)begin
  if (~reset_n) num2 <= 0;
  else if (P == S_MAIN_INIT || P == S_MAIN_PROMPT2) num2 <= 0;
  else if (received && is_num_key&&S_MAIN_READ_NUM2) num2 <= (num2 * 10) + (rx_byte - 48);
end

reg [3:0] div_cnt;
wire div_done;
assign div_done=(div_cnt==15);
reg [15:0] rem;
always @(posedge clk)begin
  if(~reset_n||P == S_MAIN_INIT)begin
    ...
    rem={rem[14:0],num1[15-div_cnt]};
    if(rem>=num2) begin
        rem=rem-num2;
        quotient[15-div_cnt]=1;
    end
    div_cnt=div_cnt+1;
  end
end
